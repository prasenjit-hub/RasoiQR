...
-- Admin Get All Restaurants (bypasses RLS for admin list, secured with admin email validation)
CREATE OR REPLACE FUNCTION admin_get_restaurants(p_admin_email TEXT)
RETURNS SETOF restaurants
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
BEGIN
  -- Verify if the calling user is a valid active admin user
  IF EXISTS (SELECT 1 FROM admin_users au WHERE au.email = LOWER(p_admin_email) AND au.is_active = TRUE) THEN
    RETURN QUERY
    SELECT * FROM restaurants
    ORDER BY created_at DESC;
  ELSE
    RAISE EXCEPTION 'Access denied. Unauthorized administrative request.';
  END IF;
END;
$$;
...
GRANT EXECUTE ON FUNCTION admin_get_restaurants TO anon, authenticated;
